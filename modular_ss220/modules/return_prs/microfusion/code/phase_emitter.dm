...
/obj/item/microfusion_phase_emitter
	...
	icon = 'modular_ss220/modules/return_prs/microfusion/icons/microfusion_gun_attachments.dmi'
	...
	var/hacked = FALSE
	/// Are we able to "hack" this emitter?
	var/can_be_hacked = TRUE
...
/obj/item/microfusion_phase_emitter/multitool_act(mob/living/user, obj/item/tool)
	if(!can_be_hacked)
		to_chat(user, span_warning("[src] doesn't look like it can be hacked!"))
		return
	if(hacked)
...
/obj/item/microfusion_phase_emitter/proc/kill()
	damaged = TRUE
	name = "damaged [name]"
	playsound(src, 'modular_ss220/modules/return_prs/microfusion/sound/overheat.ogg', 70)
	say("ERROR: Integrity failure!")
	STOP_PROCESSING(SSobj, src)

/datum/crafting_recipe/makeshift/microfusion_phase_emitter
	name = "Makeshift Microfusion Phase Emitter"
	tool_behaviors = list(TOOL_SCREWDRIVER, TOOL_WIRECUTTER, TOOL_WELDER, TOOL_MULTITOOL, TOOL_CROWBAR)
	result = /obj/item/microfusion_phase_emitter/makeshift
	reqs = list(
...
/obj/item/microfusion_phase_emitter/makeshift
	name = "makeshift microfusion phase emitter"
	desc = "An... Apparatus, While the effects on enemy combatants may be dubious, it will certainly do incredible damage to the gun's warranty. What the hell were you thinking when you came up with this?"
	max_heat = 400
	throttle_percentage = 350 // no safeguard
	can_be_hacked = FALSE
	heat_dissipation_per_tick = 10
	cooling_system_rate = 15
	integrity = 50
	color = "#503702"
