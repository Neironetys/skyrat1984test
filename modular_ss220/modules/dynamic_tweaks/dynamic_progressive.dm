...
/datum/controller/subsystem/progressive_dynamic/fire()
	if (!GLOB.dynamic_progressive_enabled || generated_threat_by_progressive_dynamic >= max_generated_threat || !SSdynamic)
		return
	...
	if (threat_increase + SSdynamic.threat_level > SSdynamic.max_threat_level)
		threat_increase = SSdynamic.max_threat_level - SSdynamic.threat_level

	SSdynamic.threat_level += threat_increase
